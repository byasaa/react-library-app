...
class Books extends Component {
  ...
  getAllBook = async () => {
    ...
    await this.props
      .dispatch(getBook(search, limit, page, order, sort, token))
      .then(() => {
        this.setState({
    ...
    if (this.props.location.search === "") {
      ...
      if (qs.page >= 1) {
        ...
        this.props.history.push(
          `${this.props.location.search}&page=${parseInt(qs.page) + 1}`
        );
      }
    }
  };
    ...
    return (
      <>
        {this.state.books.map((book) => {
          return (
            <Col key={book.id} lg={3} md={6} className="mt-3 mb-4 mb-lg-0">
              <Card className="shadow-sm border-0 rounded">
                <CardBody className="p-0">
                  <CardImg
                    src={`${process.env.REACT_APP_API_URL}img/${book.image}`}
                    alt={`${book.title}`}
                  />
                  <div className="p-4">
